...
const addressCandidates = [
  {
    roadAddress: "역삼로 215, 역삼래미안아파트",
    buildingName: "역삼래미안아파트",
    jibunAddress: "서울 강남구 역삼동",
  },
  {
    roadAddress: "대치동 998, 래미안 대치팰리스",
    buildingName: "래미안 대치팰리스",
    jibunAddress: "서울특별시 강남구 대치동 998",
  },
];
...
const PROFILES = [
  {
    ...
    icon: "🛡️",
    title: "다치지 않고 안전하게 지내고 싶어요",
    cardDescription: "계단·문턱이 적고, 병원이 가깝고, 동네가 안전했으면 해요",
  },
  {
    profileCode: "CONVENIENCE_FIRST",
    icon: "🛒",
    title: "혼자서도 편하게 지내고 싶어요",
    cardDescription: "장보기·산책·대중교통·은행이 가까웠으면 해요",
  },
  {
    profileCode: "VALUE_STABILITY",
    icon: "💰",
    title: "주거비 부담을 줄이고 싶어요",
    cardDescription: "관리비가 적고, 나중에 팔기 쉬운 집이면 좋겠어요",
  },
  {
    profileCode: "BALANCED",
    icon: "⚖️",
    title: "골고루 균형 있게 보고 싶어요",
    cardDescription: "안전·편의·비용이 균형있는 집이면 좋겠어요",
  },
];
...
function submitMortgage() {
  ...
  survey.saveMortgage({
    ...
    mortgageBalanceAmount: survey.hasMortgage
      ? survey.mortgageBalanceAmount
      : 0,
  });
}
...
const canSubmitReserve = computed(
  () =>
    survey.reserveOptionCode &&
    (!isCustomReserve.value || survey.reserveCustomAmount),
);
function submitReserveBudget() {
  ...
  survey.saveReserveBudget({
    ...
    reserveCustomAmount: isCustomReserve.value
      ? survey.reserveCustomAmount
      : null,
  });
}
...
const netProceeds = computed(
  () => (survey.maxPurchaseBudgetAmount ?? 0) + (survey.reserveAmountUsed ?? 0),
);
const mortgagePaid = computed(() =>
  survey.hasMortgage ? survey.mortgageBalanceAmount || 0 : 0,
);
...
function toggleDong(dongName) {
  ...
  const idx = list.findIndex(
    (r) => r.eupmyeondongName === dongName && r.sigunguName === gu.value,
  );
  if (idx >= 0) list.splice(idx, 1);
  else
    list.push({
      sidoName: sido.value,
      sigunguName: gu.value,
      eupmyeondongName: dongName,
    });
}
function isDongSelected(dongName) {
  return survey.desiredRegions.some(
    (r) => r.eupmyeondongName === dongName && r.sigunguName === gu.value,
  );
}
function wholeGuSelected() {
  return survey.desiredRegions.some(
    (r) => r.sigunguName === gu.value && !r.eupmyeondongName,
  );
}
function pickWholeGu() {
  survey.desiredRegions = survey.desiredRegions.filter(
    (r) => r.sigunguName !== gu.value,
  );
  survey.desiredRegions.push({
    sidoName: sido.value,
    sigunguName: gu.value,
    ...
    <div class="survey-card">
      ...
      <div v-if="survey.showIntro" class="intro-wrap">
        ...
        <h2 class="intro-heading">
          {{ survey.userName }} 님에게<br />꼭 맞는 집을 추천해드릴게요
        </h2>
      ...
      <template v-else>
        ...
        <div class="top-row">
          ...
          <div class="progress-track">
            <div
              class="progress-fill"
              :style="{ width: survey.progressPct + '%' }"
            ></div>
          </div>
          <div class="progress-frac">
            {{ survey.progressStep }} / {{ PROGRESS_STEPS_TOTAL }}
          </div>
          <button
            class="reset-link"
            :disabled="survey.loading"
        ...
        <div v-if="survey.done" class="text-center py-5">
          ...
          <p class="step-desc">
            입력하신 내용을 바탕으로 맞춤 주택을 찾아볼게요
          </p>
          <button class="primary-btn" @click="survey.reset">
            다시 설문하기
          </button>
        </div>
        ...
        <template v-else-if="currentStepName === 'current-home'">
          <h2 class="step-title">
            지금 살고 계신 집,<br />주소가 어떻게 되시나요?
          </h2>
          <p class="step-desc">도로명 주소로 검색해주세요</p>
          <input
            v-model="addressQuery"
            class="addr-input"
            placeholder="예: 서울시 강남구 역삼로 215"
          />
          <div
            v-for="c in addressCandidates"
            :key="c.roadAddress"
            class="recent-item"
            :class="{
              selected: selectedAddress?.roadAddress === c.roadAddress,
            }"
            role="button"
            tabindex="0"
            @click="selectedAddress = c"
            @keyup.enter="selectedAddress = c"
          >
            <div>
            ...
            <div
              class="check-dot"
              :class="{ on: selectedAddress?.roadAddress === c.roadAddress }"
            >
              ✓
            </div>
          </div>
          <p v-if="survey.errorMessage" class="text-danger small mt-2">
            {{ survey.errorMessage }}
          </p>
          <button
            class="primary-btn"
            :disabled="!selectedAddress || survey.loading"
            @click="submitCurrentHome"
          >
            다음
          </button>
        </template>
        ...
        <template v-else-if="currentStepName === 'preference'">
          ...
          <button
            ...
            :key="p.profileCode"
            class="pref-card"
            :class="{ selected: survey.profileCode === p.profileCode }"
            @click="survey.profileCode = p.profileCode"
          >
            ...
            <div
              class="pref-check"
              :class="{ on: survey.profileCode === p.profileCode }"
            >
              ✓
            </div>
          </button>
          ...
          <button
            class="primary-btn"
            :disabled="!survey.profileCode || survey.loading"
        ...
        <template v-else-if="currentStepName === 'mortgage'">
          <h2 class="step-title">
            지금 살고 계신 집에<br />남은 대출이 있으신가요?
          </h2>
          <p class="step-desc">
            집을 팔 때 갚아야 할 금액이라 계산에 꼭 필요합니다
          </p>
          <div class="toggle-pair">
            <button
              class="toggle-opt"
              :class="{ on: survey.hasMortgage === false }"
              @click="survey.hasMortgage = false"
            >
              대출이 없어요
            </button>
            <button
              class="toggle-opt"
              :class="{ on: survey.hasMortgage === true }"
              @click="survey.hasMortgage = true"
            >
              대출이 있어요
            </button>
          </div>
          <template v-if="survey.hasMortgage">
            <div class="mt-4 fw-bold" style="color: #8a7430; font-size: 13px">
              남은 대출 잔액
            </div>
            <div class="amount-box">
              <input
                v-model.number="survey.mortgageBalanceAmount"
                type="number"
                min="0"
              />
              <span>원</span>
            ...
            <div class="chip-row">
              <button class="chip-btn" @click="addMortgageChip(10_000_000)">
                + 1,000만
              </button>
              <button class="chip-btn" @click="addMortgageChip(50_000_000)">
                + 5,000만
              </button>
              <button class="chip-btn" @click="addMortgageChip(100_000_000)">
                + 1억
              </button>
              <button
                class="chip-btn"
                @click="survey.mortgageBalanceAmount = 0"
              >
                지우기
              </button>
            </div>
          ...
          <button
            class="primary-btn"
            :disabled="!canSubmitMortgage || survey.loading"
        ...
        <template v-else-if="currentStepName === 'reserve'">
          <h2 class="step-title">
            이사하고 나서, 최소 얼마 정도는<br />남아있으면 될까요?
          </h2>
          <div class="tier-grid">
            <button
              v-for="t in RESERVE_TIERS"
              :key="t.code"
              class="tier-card"
              :class="{ on: survey.reserveOptionCode === t.code }"
              @click="pickReserveTier(t.code)"
            >
              {{ t.label }}
            </button>
            <button
              class="custom-tier"
              :class="{ on: isCustomReserve }"
              @click="pickReserveTier('CUSTOM')"
            >
              직접 입력할게요
            </button>
          </div>
          <template v-if="isCustomReserve">
            <div class="amount-box mt-3">
              <input
                v-model.number="survey.reserveCustomAmount"
                type="number"
                min="0"
            ...
            <div
              v-if="survey.reserveCustomAmount"
              style="
                color: #b39b2e;
                font-weight: 700;
                font-size: 14px;
          ...
          <button
            class="primary-btn"
            :disabled="!canSubmitReserve || survey.loading"
            @click="submitReserveBudget"
          >
            결과 보러 가기
          </button>
        </template>
        ...
        <template v-else-if="currentStepName === 'summary'">
          <h2 class="summary-headline">
            {{ formatKRW(survey.maxPurchaseBudgetAmount) }} 이하의<br />집을
            찾아볼게요
          </h2>
          ...
          <div class="summary-stat">
            ...
            <div class="value">
              {{ formatKRW(survey.maxPurchaseBudgetAmount) }}
            </div>
          </div>
          <button class="primary-btn" @click="survey.advanceFromSummary">
            다음
          </button>
        </template>
        ...
        <template v-else-if="currentStepName === 'region'">
          ...
          <p class="step-desc mt-3 mb-2 text-start">
            구체적으로 어디가 좋을까요? 잘 모르겠으면 비워두셔도 괜찮아요
          </p>
          ...
          <div class="d-flex gap-2 flex-wrap mt-3">
            <button
              ...
              :key="d.name"
              class="region-chip"
              :class="{ on: isDongSelected(d.name), zero: d.count === 0 }"
              @click="toggleDong(d.name)"
            >
              {{ d.name }} ({{ d.count }}개)
            </button>
            <button
              class="region-chip dashed"
              :class="{ on: wholeGuSelected() }"
              @click="pickWholeGu"
            >
              구 전체로 볼게요
            </button>
    ...
    >
      <div class="reset-modal-card">
        ...
        <div class="reset-modal-actions">
          <button class="reset-btn-cancel" @click="cancelReset">
            계속 진행할게요
          </button>
          <button class="reset-btn-confirm" @click="confirmReset">
            처음부터 할게요
          </button>
        </div>
      </div>
